...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
        E = tf.get_variable("embedding", (len(self.wv), self.config.embed_size), initializer = tf.constant_initializer(0.0))
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
        ...
        h = tf.tanh(tf.matmul(tf.to_float(window), W) + b1)
    ...
    yhat = tf.nn.dropout(output, self.dropout_placeholder)
