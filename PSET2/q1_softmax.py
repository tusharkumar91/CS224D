...
def softmax(x):
  ...
  with tf.Session() as sess:
        ...
        xmax = tf.reshape(tf.reduce_max(x, axis=1), (s[0], 1)) 
        xnew = tf.exp(x - xmax)
        xexpSum = tf.reshape(tf.reduce_sum(xnew, axis = 1), (s[0], 1))
        out = xnew/xexpSum
  return out
...
def cross_entropy_loss(y, yhat):
  ...
  """
  with tf.Session() as sess:
      yshape = sess.run(tf.shape(y))
      n_smaples = yshape[0]
      costi = tf.to_float(y) * tf.log(yhat)
      cost = tf.reduce_sum( -1 * (tf.reshape(tf.reduce_sum(costi, axis = 1), (n_smaples,1))))
  return cost
...
def test_cross_entropy_loss_basic():
  ...
  yhat = np.array([[0.5, 0.5], [0.5, 0.5], [0.5, 0.5]])
