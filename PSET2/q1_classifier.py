...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    """
    with tf.variable_scope("softmax-classification"):
        tf.global_variables_initializer()
        W = tf.get_variable("weights", (self.config.n_features, self.config.n_classes), initializer = tf.constant_initializer(0.0))
        b = tf.get_variable("bias", (1 , self.config.n_classes), initializer = tf.constant_initializer(0.0))
        #print(input_data.get_shape())
        #print(W.get_shape())
        #X = tf.placeholder(tf.float32, (self.config.batch_size, self.config.n_features))
