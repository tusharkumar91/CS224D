...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
        ### YOUR CODE HERE
        E = tf.get_variable("embedding", (len(self.vocab), self.config.embed_size), initializer = tf.constant_initializer(0.0))
        embedding = tf.nn.embedding_lookup(E, self.input_placeholder)
        print(self.input_placeholder.get_shape())
        print(embedding.get_shape())
        li = tf.split(split_dim=1, num_split = self.config.num_steps, value=embedding)
#        print(len(li))
#        print(li[0].get_shape())
        for l in li:
            inputs.append(tf.squeeze(l, axis=1))
#        print(inputs[0].get_shape())
  ...
  def add_projection(self, rnn_outputs):
    ...
    ### YOUR CODE HERE
    outputs = []
    with tf.variable_scope("RNN-output") as scope:
        for idx, ht in enumerate(rnn_outputs):
            if idx is not 0:
            ...
            outputs.append(tf.matmul(ht,U) + b_2)
...
#    raise NotImplementedError
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN") as scope:
        ...
        for step in inputs:
            if count is 0:
                input_dropout = tf.nn.dropout(inputs[count], self.dropout_placeholder)
                ht = tf.sigmoid(tf.matmul(self.initial_state, H) + tf.matmul(input_dropout, I) + b_1)
            else:
                input_dropout = tf.nn.dropout(inputs[count], self.dropout_placeholder)
                ht = tf.sigmoid(tf.matmul(prev, H) + tf.matmul(input_dropout, I) + b_1)
            prev = ht
            count = count + 1
            rnnoutputs.append(tf.nn.dropout(ht, self.dropout_placeholder))
            
    ### YOUR CODE HERE
    #raise NotImplementedError
    ### END YOUR CODE
    self.final_state = rnnoutputs[-1]
    return rnnoutputs
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ### YOUR CODE HERE
    #raise NotImplementedError
    tok = np.array(tokens[-1]).reshape((-1,1))
    feed = {model.input_placeholder: tok,
