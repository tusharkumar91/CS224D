...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    sentVector = sentVector/numWords   
    return sentVector

    indices = np.zeros((wordVectors.shape[0],1))
    for word in sentence:
        indices[tokens[word], 0] = 1

    return (indices.T.dot(wordVectors))/len(sentence)
